"""``--emit-format`` works on an app's own entry point, not only on ``func`` (D-2).
...
which has both as pre-command globals", and `--emit-format` was simply absent. An
...
def emit(out: Stdout) -> None:
    "Emit a mapping so --emit-format decides the rendering."
    out.emit({"answer": 42})
"""
...
class TestTheFlagIsAcceptedOnBothSurfaces:
    ...
    def test_every_value_the_grammar_declares_is_accepted(
        ...
        result = cli_run(["--emit-format", value, "emit"], cwd=emit_tree)

        assert result.exit_code == 0, f"--emit-format {value}: {result.stderr}"
