...
def store(tmp_path: Path) -> ScopeStore:
    ...
    s = ScopeStore.for_project(tmp_path)
...
class TestTheDraftSlot:
    def test_a_gate_starts_with_no_draft(self, store: ScopeStore) -> None:
        assert store.get_gate_draft("rel-1", "approve") is None

    def test_values_round_trip(self, store: ScopeStore) -> None:
    ...
    def test_putting_replaces_rather_than_merges(self, store: ScopeStore) -> None:
    ...
    def test_an_unknown_gate_is_false_not_a_crash(self, store: ScopeStore) -> None:
...
class TestTheWalkCannotSeeADraft:
    def test_drafting_never_writes_the_payload(self, store: ScopeStore) -> None:
...
class TestReopen:
    def test_it_moves_the_payload_back_into_the_draft(self, store: ScopeStore) -> None:
    ...
    def test_reopening_an_unanswered_gate_is_false(self, store: ScopeStore) -> None:
        assert store.reopen_gate("rel-1", "approve") is False

    def test_reopening_an_unknown_gate_is_false(self, store: ScopeStore) -> None:
        assert store.reopen_gate("rel-1", "nope") is False

    def test_the_store_applies_no_policy(self, store: ScopeStore) -> None:
...
class TestDeleteScope:
    def test_it_removes_the_scope(self, store: ScopeStore) -> None:
