...
def store(tmp_path: Path) -> FreshStore:
    ...
    s = FreshStore.for_project(tmp_path)
...
class TestTheDraftSlot:
    def test_a_gate_starts_with_no_draft(self, store: FreshStore) -> None:
        assert store.get_gate_draft("rel-1", "approve") is None

    def test_values_round_trip(self, store: FreshStore) -> None:
    ...
    def test_putting_replaces_rather_than_merges(self, store: FreshStore) -> None:
    ...
    def test_clearing_removes_the_key(self, store: FreshStore, tmp_path: Path) -> None:
    ...
    def test_an_unknown_gate_is_false_not_a_crash(self, store: FreshStore) -> None:
...
class TestTheWalkCannotSeeADraft:
    def test_drafting_never_writes_the_payload(self, store: FreshStore) -> None:
...
class TestReopen:
    def test_it_moves_the_payload_back_into_the_draft(self, store: FreshStore) -> None:
    ...
    def test_reopening_an_unanswered_gate_is_false(self, store: FreshStore) -> None:
        assert store.reopen_gate("rel-1", "approve") is False

    def test_reopening_an_unknown_gate_is_false(self, store: FreshStore) -> None:
        assert store.reopen_gate("rel-1", "nope") is False

    def test_the_store_applies_no_policy(self, store: FreshStore) -> None:
...
class TestDeleteScope:
    def test_it_removes_the_scope(self, store: FreshStore) -> None:
        assert store.delete_scope("rel-1")
        assert store.scope_ids() == []

    def test_deleting_an_absent_scope_is_false(self, store: FreshStore) -> None:
        assert store.delete_scope("nope") is False

    def test_it_is_a_hard_delete_with_no_backup(
        self, store: FreshStore, tmp_path: Path
    ) -> None:
