...
class TestScopeRecords:
    def test_ensure_scope_is_idempotent(self, store: FreshStore) -> None:
        store.scopes.ensure_scope("s1", workflow="deploy")
        store.scopes.ensure_scope("s1")
        scope = store.scopes.get_scope("s1")
    ...
    def test_unknown_scope_reads_none(self, store: FreshStore) -> None:
        assert store.scopes.get_scope("nope") is None
        assert store.scopes.get_step("nope", "k") is None
        assert store.scopes.get_branch("nope", "src") is None
        assert store.scopes.get_gate("nope", "g") is None
        assert store.scopes.get_position("nope") is None
        assert store.scopes.get_epilogue("nope") is None
    ...
    def test_step_record_round_trip(self, store: FreshStore) -> None:
        ...
        store.scopes.record_step("s1", "build::h1", record)
        assert store.scopes.get_step("s1", "build::h1") == record

    def test_steps_are_scoped(self, store: FreshStore) -> None:
        store.scopes.record_step("s1", "build::h1", {"status": "success"})
        assert store.scopes.get_step("s2", "build::h1") is None
    ...
    def test_branch_choice_round_trip(self, store: FreshStore) -> None:
        store.scopes.record_branch("s1", "check", "deploy")
        assert store.scopes.get_branch("s1", "check") == "deploy"

    def test_epilogue_round_trip(self, store: FreshStore) -> None:
        store.scopes.record_epilogue("s1", {"status": "success", "return_value": 7})
        assert store.scopes.get_epilogue("s1")["return_value"] == 7

    def test_scope_ids_sorted(self, store: FreshStore) -> None:
        store.scopes.ensure_scope("b")
        store.scopes.ensure_scope("a")
        assert store.scopes.scope_ids() == ["a", "b"]
...
class TestGates:
    def test_gate_round_trip(self, store: FreshStore) -> None:
        ...
        store.scopes.put_gate("s1", "approve", record)
        assert store.scopes.get_gate("s1", "approve") == record

    def test_deposit_payload(self, store: FreshStore) -> None:
        store.scopes.put_gate("s1", "approve", {"model": "Approval", "payload": None})
        assert store.scopes.deposit_gate_payload("s1", "approve", {"ok": True}) is True
        assert store.scopes.get_gate("s1", "approve")["payload"] == {"ok": True}

    def test_deposit_to_unknown_gate_reports_false(self, store: FreshStore) -> None:
        assert store.scopes.deposit_gate_payload("s1", "nope", {"ok": True}) is False

    def test_position_round_trip(self, store: FreshStore) -> None:
        store.scopes.set_position("s1", "approve")
        assert store.scopes.get_position("s1") == "approve"
        store.scopes.set_position("s1", None)
        assert store.scopes.get_position("s1") is None
...
class TestScopeBatch:
    ...
    def test_scope_batch_writes_once_and_persists(self, store: FreshStore) -> None:
        with store.scopes.batch():
            store.scopes.ensure_scope("s1", "release")
            store.scopes.set_position("s1", "approve")
        reloaded = FreshStore(store.substrate)
        assert reloaded.scopes.get_position("s1") == "approve"
    ...
    def test_nested_batch_reuses_outer(self, store: FreshStore) -> None:
        with store.scopes.batch(), store.scopes.batch():
            store.scopes.ensure_scope("s1")
        assert FreshStore(store.substrate).scopes.scope_ids() == ["s1"]

    def test_batch_preserves_existing_records(self, store: FreshStore) -> None:
        store.scopes.ensure_scope("pre")
        with store.scopes.batch():
            store.scopes.ensure_scope("new")
        assert store.scopes.scope_ids() == ["new", "pre"]
...
class TestTwoFiles:
    ...
    def test_a_state_version_bump_leaves_scopes_intact(self, store: FreshStore) -> None:
        ...
        store.scopes.ensure_scope("s1", "release")
        store.scopes.put_gate("s1", "approve", {"payload": {"approved_by": "sam"}})
        store.put_fingerprint("k", {"n": 1})
        ...
        gate = store.scopes.get_gate("s1", "approve")  # the record: survives
...
class TestClear:
    def test_clear_resets_derived_state_but_keeps_scopes(
        ...
        assert store.scopes.scope_ids() == ["s1"]
    ...
    def test_clear_returns_none_when_there_were_no_scopes(
        ...
        store.clear()
        assert store.scopes.clear() is None
