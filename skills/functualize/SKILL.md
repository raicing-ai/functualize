...
description: >
  ...
  defining functualize jobs, when a job is not being discovered, when `func`
  is not found or you are unsure which environment it lives in, when wiring
  job config or credentials, or when the user mentions functualize, `func
  run`, `func builtin`, RunContext, or @job.
license: MIT
metadata:
  version: "0.1.0"
  project: functualize
...
detected context (`uv add functualize`, `poetry add functualize`), and let the
human decide. Never `pip install` bare, never `--user`, never system Python —
it appears to succeed, fixes nothing, and leaves the machine dirtier.
...
`out.emit()` respects `--output` (`json`, `ndjson`, `raw`, `none`), so the same
job is human-readable and machine-parseable without branching.
...
config file, and are declared with `Secret[str]` so they render as `•••`
everywhere — logs, perf reports, event payloads, tracebacks, emitted output.
