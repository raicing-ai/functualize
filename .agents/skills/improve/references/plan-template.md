...
| E2E TUI   | `verify-e2e --tier smoke` (skill invocation) | PASS (optional — include when plan touches TUI-impacting code) |
...
**Reaches production via**: `<caller → callee chain>` — the path by which a real
invocation exercises this. Write "N/A (wiring lands in Step N)" if this step
deliberately adds an unreachable component, so the gap is visible rather than
assumed. A step whose only caller is a test is not finished; see
...
**Never end a plan on an unwired component.** If Step N builds something and
Step N+1 connects it, both must be in the same plan — a plan that stops after
Step N ships tested, unreachable code and reports success. That has happened
three times in this codebase; see `contributor/guides/wiring-discipline.md`.

### E2E verification gates (optional)

When a plan touches TUI-impacting code (kernel layers, public API used by
`_cli/`, or `_cli/` itself), annotate milestone steps with a verification
gate. The executor invokes the `verify-e2e` skill at the specified tier
before proceeding past that step.

Annotation syntax — append to the step title:

```markdown
### Step 3: Refactor ResolutionChain [verify-e2e:smoke]
### Step 5: Update ConfigTablePanel display [verify-e2e:targeted]
```

Tiers:
- `smoke` — TUI boots, one job executes, no crash (~15s)
- `targeted` — specific surface downstream of the change is probed (~60s)
- `full` — all behavioral claims validated (~3-5min, usually reserved for Done)

Use gates after kernel/infrastructure steps that subsequent TUI steps build
upon. If the smoke gate fails, the executor treats it as a STOP condition.

Read `.agents/skills/verify-e2e/SKILL.md` for full details.
...
- [ ] (If plan touches TUI-impacting code) `verify-e2e --tier smoke` reports PASS
