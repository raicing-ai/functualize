...
With functualize-inline installed, `rc.prompts.ask` renders real terminal
...
def run(rc: RunContext) -> str:
    ...
    if not rc.prompts.confirm("Deploy to production?", destructive=True, default=False):
        rc.log("Deploy cancelled")
        return "cancelled"

    region = rc.prompts.choice(
        "Which region?",
