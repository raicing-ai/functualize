...
- **12 official plugins** live in the `plugins/` workspace, grouped by what they serve (`plugin-taxonomy`): `adapters/` (http, lambda, mcp, flow-viz, inline), `substrates/` (substrate-sqlite), `credentials/` (aws, bitwarden) and `domains/`, where each implementation is a *sibling* of the contract it implements (ai + ai-pydantic, tasks + tasks-local). Every plugin sits at exactly two levels below `plugins/`, which is the depth `members = ["plugins/*/*"]` and `.claude/hooks/spec_gate.py` are taught.
  - Previously recorded as "13 … (ai, interactivity, state, tasks)". There is no `interactivity` domain and never was; `functualize-state` was removed by ADR-022; `fullscreen-tui` does not exist at any depth. Measured: `ls -d plugins/*/functualize-*/ | wc -l` → 12.
