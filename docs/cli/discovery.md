...
These apply on *read* rather than on write, so the discovery cache stays a superset of what any one configuration admits and a changed `require_job_*` setting takes effect immediately. File-level filters decide what gets *written* to the cache, so they cannot work that way — instead the cache header fingerprints your effective discovery configuration, and changing any of it discards the cache and rescans. Either way a filter change takes effect on your next command; `func builtin cache clear` is not needed.
