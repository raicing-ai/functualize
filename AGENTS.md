...
| Debugging or manually verifying live TUI/CLI behavior (seeing the rendered screen, driving the app, trying examples) | `.claude/skills/observe-tui/SKILL.md` (PTY screen probe via pyte + tmux session driving). Manual/agent verification only — never wire it into automated tests or CI |
