...
| File | Size | Change |
|---|---|---|
| `src/functualize/_types/persistence.py` | ~430 | the workspace port, separate from RuntimeStore |
| `contributor/adr/` | new | an ADR recording the AgentFS evaluation outcome |
...
- **Divergent change** risk: two ports over possibly one file means a change to storage layout may touch both. Accepted if physical co-location is chosen; re-examine if it is not.
