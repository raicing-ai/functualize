...
| File | Size | Change |
|---|---|---|
| `src/functualize/_engine/recording/` | ~440 | + input_recorder |
| `src/functualize/_primitives/outbox.py` | new ~200 | the dispatcher |
| `src/functualize/_engine/workflow_walker.py` | 1153 | gate deposit and reopen through the transaction |
...
- **Open question, needs maintainer review**: does the dispatcher run inline after commit, as a managed background component, or as a separate worker? Design 1 listed this open and it still is. A background dispatcher is a second thing that runs work, which interacts with the single JobExecutionEngine path.
