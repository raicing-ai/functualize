...
**Test:** the **two** plugin `test_status_codes.py` suites (http, lambda; MCP has none — it carries `test_terminal_affinity.py`) — already parametrized over every
...
| AC-2 no surface defines its own success set | **Corrected after review (roa B3/S1).** The original row claimed "zero hits outside the module" on an `rg` for the three members *on one line*; `ruff format` had wrapped them across three, and `_engine/capabilities/invoke.py` held exactly that set. It also listed **five** adjudicated sites, of which only two were `(SUCCESS, SKIPPED)` sets and two (`capabilities/workflow.py:166`, `:199`) are `(FAILURE, REFUSED)` hook-firing conditions that no reading of the file makes a success set — so a reader could not tell which sites were actually read. The honest inventory: **two** graph-edge sets in `executor.py` (the dependency scheduler and the walk, both with their call path in a comment); the `invoke.py` set was a **delivery** decision after all — reached only through the CLI's parallel observer, controlling a `::error::` annotation — and moved to `_cli/parallel_output.py`. Now kept by `tests/types/test_no_second_failure_set.py`, an AST scan with a named allowlist, so formatting is not a variable and every survivor carries its reason |
| AC-3 each surface names its family, greppably | **Corrected after review (roa S2).** Three of six surfaces named it only in a **docstring**, and T6's gate (`rg -c 'Family.WIRE' <file>` ≥ 1) was satisfied by that prose — which even asserted the property the gate was meant to check. `Family.WIRE` and `Family.TOOL` therefore reached HEAD with **no code consumer anywhere**. The three plugin surfaces now declare `OUTCOME_FAMILY = Family.<X>` as an assignment, and `tests/types/test_every_surface_declares_its_family.py` checks each declaration against what the surface actually renders for BLOCKED — the one status the families disagree about. Code declarations: `_cli/builtins.py` (PROCESS), `_cli/tui/job_execution.py` (PANEL **and** PROCESS — the one surface with two), `app/adapters/click_params.py` (PROCESS), plus http/lambda (WIRE) and mcp (TOOL) |
...
| AC-7 panel ≡ table parity derived from the module | **Corrected after review (roa B2).** `tests/types/test_outcome_families.py` tests the *module* and never touches the panel; nothing ran a BLOCKED run through the TUI at all, so T7's declared sabotage had nothing to fail. Now `tests/tui_audit/test_panel_agrees_with_table.py` — every terminal status × two assertions, through a real TUI and the real thread worker, with expectations derived from `functualize.types` |
| AC-8 plugin `test_status_codes.py` unchanged | **Two** suites, not three (roa S6): http 33 passed · lambda 32 passed. MCP has no `test_status_codes.py`; it carries `test_terminal_affinity.py` instead. `spec.md` was corrected during execution and this table and T6's text were not |
...
| AC-10 every consumer reads it through the module | `tests/types/test_flag_grammar_consumer_count.py` — pins the consumer set and prints it on failure. **Scope corrected after review (roa S4/S5):** it is a *mention* scan bounded to `src/`, so it pins mentions rather than dependencies, and it cannot see a sixth copy (a new file with its own table and its own names) or a consumer under `tests/`/`plugins/`. One such consumer existed — `tests/adapters/test_boolean_negation.py` imported `negative_flag_for` from the `_types/naming.py` re-export "kept for older importers", and it was the only importer. Migrated to `functualize.types`; the re-export deleted (clean cutover, pre-release). Two import-level tests added beside the mention scan |
...
- the two plugin `test_status_codes.py` suites (http, lambda), unchanged — corrected from "three" after review (roa S6); `find plugins -name 'test_status_codes.py'` returns two
...
| 2 | Four consumers stopped deciding, and only one answer changed. | ~~`rg -c 'RunStatus.SUCCESS, RunStatus.SKIPPED, RunStatus.BLOCKED' src/functualize/ --glob '!**/outcome.py'` must be 0.~~ **superseded** — that single-line pattern could not see the set `ruff format` had wrapped across three lines in `_engine/capabilities/invoke.py`, and it hid a real divergence (roa B3). Replaced by `uv run pytest tests/types/test_no_second_failure_set.py`, an AST scan. Then verify the **changed** answers are the two named in the T4 commit and no others — run the two plugin suites and confirm byte-identical outcomes. | Revert `_resume_exit`'s gate-vocabulary translation; `tests/integration/test_cli_workflow_parity.py::test_resume_with_incomplete_input_does_not_advance` must fail. |
...
- **W3 is alone because it owns the only change to a surface's *family choice*.** Corrected
  after review (roa S3): the original sentence said "the only behaviour change in the
  feature", and wave 2 deliberately changed **two** answers — `func builtin parallel` now
  exits 5 for a BLOCKED job where it exited 0 (`_cli/builtins.py`, whose own comment says
  "This changes one answer, deliberately"), and `_resume_exit` now answers `ExitCode.USAGE`
  for an unrecognised status string where it answered 0 or 1. The wave-audit table already
  knew this — it asks wave 2 to "verify the changed answers are the two named in the T4
  commit and no others" — so the graph's rationale contradicted the audit in the same file,
  and a reviewer bisecting for the parallel exit-code change would land on W3 and find
  nothing. Isolating W3 means a
