...
| File | Size | Change |
|---|---|---|
| `tests/substrate_probe/` | new | one module per backend, ~200 lines each |
| `tests/substrate_probe/fakes.py` | new | BatchOnlySqliteDriver, FakeObjectStore, FakeItemStore — Tier A, no Docker, no creds |
| `contributor/architecture/research/substrate-capability-matrix.md` | new | the committed results table |
...
- **Duplicated code** across the per-backend probe modules is accepted: each is a standalone measurement whose value is being readable in isolation. Sharing them prematurely would hide the differences the probe exists to find.
