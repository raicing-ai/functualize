# START HERE — FUN-24: Repair the four runtime persistence defects
...
| | |
|---|---|
| **Jira** | FUN-24 — https://raicing-ai.atlassian.net/browse/FUN-24 |
| **Branch** | `fix/runtime-persistence-defects` |
| **Wave** | 0 of 7 |
| **Blocks** | FUN-17, FUN-18, FUN-19, FUN-20, FUN-21, FUN-22, FUN-23 |
| **Runs in parallel with** | FUN-25 (probes vendors, needs nothing from here) |
...
1. 03-the-four-defects.md — the reproductions, run them first
2. 02-what-exists-today.md §3, §4, §5 — the two safety mechanisms and their non-use
3. ../durability-outsourcing/06-s3.md §3 — why Stored.revision must become opaque
...
```bash
cd /home/ubuntu/orca/workspaces/functualize/rp-24-defect-repair
uv sync
uv run pytest -q                   # confirm green before you change anything
cat .spec/features/runtime-persistence-repair/tasks.md
```
...
`.spec/STATUS.md` or `contributor/adr/` and `git rm -r .spec/features/runtime-persistence-repair`.
