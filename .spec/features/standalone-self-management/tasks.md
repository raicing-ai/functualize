...
- [x] **1.1** `runtime.py`: add `Detection.standalone_binary`, read `PYAPP` by
...
- [x] **1.2** Build inputs: `PYAPP_PASS_LOCATION: "1"` in the `binaries` job and
...
- [x] **2.1** `package_ops.py`: `script_name(environ)` resolving
...
- [x] **2.2** `manifest.resolve_binary_path` prefers the `PYAPP` path over
...
- [x] **3.1** `self_update.py`: `ReleaseSource`, `read_release_source`,
...
- [x] **3.2** Wire it: `self_cmd.update` catches `StandaloneUpdateError` and calls
...
- [x] **4.1** Cross-surface parity for the newly reachable commands, and AC14's
      regression guard for the untouched modes.
      Files: `tests/_cli/test_self_management_surfaces.py`

- [x] **4.2** Documentation: replace the known-gap warnings with the real
