...
- A commit message, a PR title, or a PR body carrying a tracker key or tracker URL, an internal task/run/workspace identifier, or a machine identity (an agent, model, harness or automation account named as an author, which arrives as a `Co-authored-by:` trailer) — reason: a squash merge publishes the title as the subject and the body as the body, so all three are the permanent, public record of `master`. The tracking context and the tooling that produced a change belong in the tracker conversation, not in the record. Reported by the `message-hygiene` CI check; see `CONTRIBUTING.md` § *What a message must not carry*.
