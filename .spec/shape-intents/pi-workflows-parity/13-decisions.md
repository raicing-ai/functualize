...
| **D1e** | **Remove the early-parse `--scope-id` — as a refusal, not a deletion.** Keep it recognised so its value is still consumed, and answer it with exit 2 and the corrected post-command form. Per-command coverage verified across all five invocation paths, cold and warm ([15](15-scope-id-early-parse-removal.md)). | It is the only member of `_GLOBAL_OPTIONS_ALWAYS_VALUE` addressing persisted state rather than discovery/config/perf, every generated hint and doc already teaches the post-command spelling, and the one historical bug in this area was the global's own plumbing. **Retracts** this folder's earlier claim that the global is the only spelling guaranteed on every dispatch mode. | ✅ |
