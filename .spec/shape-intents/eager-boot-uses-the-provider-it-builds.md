# Shape Intent: Eager Boot Uses The Provider It Builds

**Status: specified, not yet implemented**
**Date: 2026-09-07**
**Scope: `_app/boot.py` `resolve_and_register_jobs`, and the second directory
scanner it reaches for instead of the provider `boot_standard` already built.
Four defects, one root cause. Paused by decision after a third round of audit
reordered the severity — recorded here so the pause is a decision, not an
omission.**

Every number below was measured against `6699af6`, not inferred. The spec and
contracts were written and verified against the live code as
`.spec/features/eager-boot-uses-the-provider-it-builds/`, then migrated here
when that directory was cleared for merge (PR #29).

**Read [`.spec/STATUS.md`](../STATUS.md) #32 first.** It is the only defect in
this area a `func` user can reach, and its fix — the collision check in
`register_descriptors`, which both boot paths call — is also what unblocks the
work below, because routing the eager path through the provider would otherwise
delete the only place that check currently lives.
