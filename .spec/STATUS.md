...
6. **`RunContext.log()` bypasses the injected `Log`** — **resolved** (`fix/runcontext-log-di`). `RunContext.log()` now resolves `Log` from the DI registry once per context and falls back to the job's stdlib logger when no `Log` is registered (the production path, where the engine builds `Log` per invocation). `TestRunContext.captured_logs()` observes `rc.log(...)`; the 0.1.0 known limitation is lifted.
