...
| Runtime persistence ports (FUN-17) | Waves 0–6 done; wave 7 (T12) landed, unticked; waves 8–10 open | `feat/runtime-persistence-ports`: the engine is constructed *with* a `RuntimeStore` at boot step 6.5 and has no path to storage it was not given. **Decided 2026-09-24 (TD-1), durable past the branch:** a storage plugin *offers* its backend from its registration call (`PluginHost.offer_substrate`, the port's twelfth member) and `_select_runtime_store` *asks* for it after configuration resolves and before selection — ADR-027's "split selection from construction", not a config event, because `invoke_config_event` swallows and storage has no safe default. `install_substrate` and `offer_substrate` are both claims; two claims refuse with `SubstrateInstallError` naming every claimant, never "first wins". Every storage door is loud: the post-selection install refusal is a `SubstrateInstallError` (so `APP_READY` loops stop swallowing it) and both registration loops re-raise it. `JsonFileSubstrate.for_project` resolves its directory on first document access, which keeps `boot_static` at zero filesystem IO. |
...
Resolution is decided by **presence, not openability**: the vault holding
nothing for a key falls through as it always did, and the vault holding
something refuses when it cannot be opened. Falling past a stored value to an
environment variable hands the job a different secret than the one provisioned,
while the run reports success — the substitution ADR-016 exists to prevent,
reachable through the one door it left open. ADR-016 §7's fall-through is
unchanged for the case it was actually about: a *miss*.

**Rules that outlive the feature:**

- **A seam whose implementations disagree about scope has no scope.** The
  keychain key provider was project-scoped and the environment one user-scoped,
  so which applied depended on whether `$FUNCTUALIZE_VAULT_KEY` happened to be
  exported — non-interactive providers are consulted first. Exporting it once
  re-encrypted every project's store under the shared key, and unsetting it
  later stranded them all. Two guarantees that contradict each other are worth
  less than one.

- **Eligibility is what the system can deliver, not what it can name.**
  `job_input_schema` publishes `config_fields or parameters`, so a `Secret[str]`
  *parameter* appears in the published schema — while config resolution only
  ever walks `config_class.model_fields`. A path can be perfectly well-formed
  and structurally undeliverable; storing against one produces a secret that
  silently does nothing.

- **A refusal must reach the user as a line.** The vault refusal is raised
  during config resolution, outside the try that builds a `JobResult`, so it
  escaped to the process boundary as a ~20-frame traceback with exit 1 — every
  test green, because they assert the exception. This is the second time that
  has happened at this exact seam; `prelude_refusal`'s docstring already
  recorded the first, for the agent-step errors. **New error types raised below
  `engine.run()` need an arm there, and the way to find out is to run the CLI.**

- **An `Any` return annotation blinds the orphan scan.** `find_referencing_
  symbols` resolves call targets through return types, so `_open_store() -> Any`
  made `SecretsVault.delete` look like it had no production caller. The scan is
  a repo discipline; annotating concretely is what keeps it honest.

- **Write the migration test against the old schema written out in full.** A
  migration test whose "before" shape is generated by the new code drifts
  silently the moment the schema changes again, and asserts nothing about the
  upgrade it names.

**What sabotage caught that a green suite did not.** Three tests passed under
the very regression they claimed to cover: an upgrade guard whose removal
silently relabelled direct entries as provider-written (making them clobberable
by `sync`), a "validated before input is read" test that asserted only that no
file appeared, and a "without a rebuild" test that asserted only the version
stamp. All three were rewritten to fail under their own sabotage.
