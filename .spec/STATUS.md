...
| [`subject-modeling/`](shape-intents/subject-modeling/) — **a directory, landed 2026-09-08 on `spec/subject-modeling`** | **Subject modeling as a functualize practice**, plus the design of its first consumer. One subject, one class; the substrate defines presence; actions are verb Protocols; the target carries identity into records and fingerprints. functualize's half is small and deliberately opinion-free: a proposed `docs/guides/subjects.md` (in `guide/`) that names the axes and the tests but **never a concrete substrate, action or target** — the moment it does, functualize has acquired an opinion about what jobs mean. The rest is **risekit**, the first third-party distribution over the hosting seams from #29, carried as the evidence the practice works. 69 acceptance criteria scored (58 met, 9 reduced, 1 partial, 1 unmet); 19 runnable probes in `evidence/`. Eleven upstream asks, **1–8 landed** (#29, #30); **asks 9, 10 and 11 open** — re-export `StaticProvider` from `functualize.plugin`; the display-provider dedupe that discards a project's override silently; and the first-party top level, where `builtin` is protected but `mcp` (new at #33) is claimable and collides silently. **Re-run drift at `787035e`: probe 11 falsifies "duplicate bare names are fatal"** — a collision now keeps the first job and silently drops the second (defect #32), which makes qualified names load-bearing rather than merely tidy. |
