...
and it is called at both writers that store a status: `ScopeStore.set_scope_status` (`SCOPE`) and
`RunStore.close_run` (`RUN`), each reading the current value inside the batch it is already writing.
Each store has exactly one call, and `_types/persistence.py` is unchanged at 700 lines.
